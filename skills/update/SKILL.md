...
4. Re-validate and finalize.
   ...
   - After the commit, run `npx llmdoc fingerprint --update <paths...>` for scoped work or `npx llmdoc fingerprint --all` for a full-repository verification, then amend the resulting `meta.json` change into the same commit (`git commit --amend --no-edit -- llmdoc/`).
