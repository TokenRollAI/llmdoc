...
1. If `llmdoc/` exists, start with the CLI retrieval protocol instead of broad file crawling:
   ...
   - `npx --no-install llmdoc index` for the relevant topic or kind
   - `npx --no-install llmdoc context --files ...` when the task names code paths
   - `npx --no-install llmdoc search ...` when the task names concepts instead of paths
   - `npx --no-install llmdoc show <path...>` only for the few documents that still matter
2. If `llmdoc/` does not exist, inspect manifests, entrypoints, tests, runtime config, and release surfaces directly.
3. For `/llmdoc:update`, treat the caller's `npx --no-install llmdoc delta` result as the source of truth for scope unless it is missing or obviously insufficient.
